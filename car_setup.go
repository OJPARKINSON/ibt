...
func ParseCarSetup(sessionInfo *headers.Session) *CarSetup {
	...
	for categoryName, category := range sessionInfo.CarSetup {
		...
		for subCategoryName, subCategory := range category.(map[string]any) {
			for setupItemName, setupItemValue := range subCategory.(map[string]any) {
...
func ParseSetupItem(input string) []CarSetupItemParsedValue {
	// Handle multiple values - such as tire wear/temperature etc
